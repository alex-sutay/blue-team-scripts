#!/bin/bash
#backup mysql databases, put them in ~/database
...
for DB_NAME in $DATABASES; do
   mysqldump -u"$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" > "$BACKUP_DIR/$DB_NAME-$TIMESTAMP.sql"
    ...
    if [ $? -eq 0 ]; then
        echo "Backup completed successfully for $DB_NAME: $BACKUP_DIR/$DB_NAME-$TIMESTAMP.sql"
    else
        echo "Backup failed for $DB_NAME"
    fi
done

#TO RESTORE ALL YOU NEED TO DO IS DO (as root) IS "mysql [database-name] < [backup].sql", making a script for this now